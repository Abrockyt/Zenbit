...
export default function CoinDetailScreen({ navigation, route }) {
  ...
  return (
    <Screen footer={
      ...
      <Header title={coin ? `${coin.symbol?.toUpperCase()}/USD` : "—"} onBack={() => navigation.goBack()} right={<IconButton icon="star" onPress={toggleWatch} size={18} />} />
      ...
      {error ? (
        ...
        <View>
          ...
          <View style={{ flexDirection: "row", gap: 20, borderBottomWidth: 1, borderBottomColor: colors.borderSubtle, paddingBottom: 10, marginVertical: spacing.md }}>
            {["Price", "Info"].map((t) => (
