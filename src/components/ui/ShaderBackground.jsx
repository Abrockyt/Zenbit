...
vec3 shade(vec2 uv, vec2 p, float t) {
  ...
  for (int i = 0; i < 7; i++) {
    ...
    vec2 center = vec2(sin(t * (0.19 + fi * 0.037) + fi * 2.1 + u_seed),
                       cos(t * (0.16 + fi * 0.043) + fi * 1.4)) * 0.63;
  ...
  return palette(clamp(surface + field * 0.05, 0.0, 1.0));
}
...
const UNIFORMS = {
  colors: [[0.05, 0.1, 0.08], [0.03, 0.08, 0.06], [0.02, 0.15, 0.09], [0.0, 0.05, 0.02], [0.1, 0.18, 0.1]], // Changed colors to dark forest green for Zenbit Pro
  colorCount: 5,
  scale: 1.920,
  intensity: 0.860,
  paramA: 0.320,
